...
static inline void __set_vio_state(struct vlmc_io *vio,
...
static int doubleup_queue(struct volume_info *vi)
{
    ...
    if (!newq) {
        XSEGLOG2(&lc, E,
                 "Doubling up queue of volume %s failed. Allocation error",
                 vi->name);
        return -1;
    }
                                           ...
                                           uint32_t targetlen)
{
    ...
    buf[targetlen] = 0;
    XSEGLOG2(&lc, D, "looking up volume %s, len %u", buf, targetlen);
...
static int insert_volume(struct vlmcd *vlmc, struct volume_info *vi)
{
    ...
    XSEGLOG2(&lc, D,
             "Inserting volume %s, len: %d (volume_info: %lx) completed",
             vi->name, strlen(vi->name), (unsigned long) vi);
...
static int remove_volume(struct vlmcd *vlmc, struct volume_info *vi)
{
    ...
    if (r < 0) {
        XSEGLOG2(&lc, W,
                 "Removing volume %s, len: %d (volume_info: %lx) failed",
                 vi->name, strlen(vi->name), (unsigned long) vi);
    } else {
        XSEGLOG2(&lc, D,
                 "Removing volume %s, len: %d (volume_info: %lx) completed",
                 vi->name, strlen(vi->name), (unsigned long) vi);
    }
    return r;
}
...
static int conclude_pr(struct peerd *peer, struct peer_req *pr)
{
    ...
    if (vi) {
        ...
        XSEGLOG2(&lc, D,
                 "vi: %lx, volume name: %s, active_reqs: %lu, pending_pr: %lx",
...
static int do_accepted_pr(struct peerd *peer, struct peer_req *pr)
{
    ...
    if (should_freeze_volume(pr->req)) {
        ...
        if (vi->active_reqs) {
            ...
            XSEGLOG2(&lc, I,
                     "Active reqs of %s: %lu. Pending pr is set to %lx",
...
static int append_to_pending_reqs(struct volume_info *vi, struct peer_req *pr)
{
    ...
    if (r == Noneidx) {
        if (doubleup_queue(vi) < 0) {
            XSEGLOG2(&lc, E,
                     "Appending pr %lx to vi %lx, volume name %s failed", pr,
                     vi, vi->name);
            return -1;
        }
                           ...
                           struct xseg_request *req)
{
    ...
    if (vi->flags & VF_VOLUME_FROZEN) {
        XSEGLOG2(&lc, I,
                 "Volume %s (vi %lx) frozen. Appending to pending_reqs",
                 vi->name, vi);
...
static int mapping_info(struct peerd *peer, struct peer_req *pr)
{
    ...
    if (vio->mreq->state & XS_FAILED) {
        XSEGLOG2(&lc, E, "Info req %lx failed", (unsigned long) vio->mreq);
        vio->err = 1;
    } else {
        ...
        struct xseg_reply_info *xinfo =
            (struct xseg_reply_info *) xseg_get_data(peer->xseg, vio->mreq);
...
static int mapping_open(struct peerd *peer, struct peer_req *pr)
{
    struct vlmc_io *vio = __get_vlmcio(pr);
    if (vio->mreq->state & XS_FAILED) {
        XSEGLOG2(&lc, E, "Open req %lx failed", (unsigned long) vio->mreq);
        vio->err = 1;
    }
    xseg_put_request(peer->xseg, vio->mreq, pr->portno);
    vio->mreq = NULL;
...
static int mapping_close(struct peerd *peer, struct peer_req *pr)
{
    ...
    struct vlmc_io *vio = __get_vlmcio(pr);
    if (vio->mreq->state & XS_FAILED) {
        XSEGLOG2(&lc, E, "Close req %lx failed", (unsigned long) vio->mreq);
        vio->err = 1;
    }
    char *target = xseg_get_target(peer->xseg, pr->req);
    struct volume_info *vi = find_volume_len(vlmc, target, pr->req->targetlen);
...
static int mapping_readwrite(struct peerd *peer, struct peer_req *pr)
{
    struct vlmcd *vlmc = __get_vlmcd(peer);
    struct vlmc_io *vio = __get_vlmcio(pr);
    struct xseg_reply_map *mreply =
        (struct xseg_reply_map *) xseg_get_data(peer->xseg, vio->mreq);
    ...
    for (i = 0; i < vio->breq_len; i++) {
        ...
        breq =
            xseg_get_request(peer->xseg, pr->portno, vlmc->bportno, X_ALLOC);
        if (!breq) {
            vio->err = 1;
            break;
        }
